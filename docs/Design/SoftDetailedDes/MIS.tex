...
\section{MIS of Image Processing Module}\label{OCR_Processing_Module}
...
\begin{center}
  \begin{table}[H]
    \caption{Image Processing Module Access Programs}
...
\noindent \textbf{processImage}(image: binary):
\begin{itemize}
\item input: A valid image file in a supported format.
\item precondition: N/A
\item transition: Preprocesses the image (see
\hyperref[imgProc_preprocess]{preprocess()}) and converts it into structured
text data (see \hyperref[imgProc_extract]{extractText()}).
\item output: Structured text data extracted from the image.
\item postcondition: N/A
\item exception: Throws a FileError if the image cannot be read or an unsupported format is provided.
\end{itemize}

\noindent \textbf{validateImage}(image: binary):
\begin{itemize}
\item precondition: N/A
...
preprocess(image: binary):\label{imgProc_preprocess}
\begin{itemize}
\item input: A valid image file in a supported format.
\item transition: This function applies pre-processing steps to the image, such
as resizing, noise reduction, or thresholding, before OCR is applied.
\item output: An image object.
\end{itemize}

\noindent extractText(image: binary):\label{imgProc_extract}
\begin{itemize}
\item input: A valid image object.
\item transition: This function uses an OCR library to extract raw text from the
pre-processed image.
\item output: A string containing the extracted text.
\end{itemize}
...
\hline
% \wss{accessProg} & - & - & - \\\
categorize & item: \hyperref[expense_module]{Expense} & category: string &
InvalidInputError \\
\hline
\end{tabular}
\end{table}
...
\noindent categorize(items: List[\hyperref[expense_module]{Expense}]):
\begin{itemize}
\item precondition: The items in the list are valid
\hyperref[expense_module]{Expense} objects, that do not have a category
assigned.
\item transition:
\[
\forall \text{item} \in \text{items}, \text{item.category} := \text{category}
\]
\item output: List[\hyperref[expense_module]{Expense}] -- a list of items with
their categories set
\item postcondition: The items in the list have their categories set.
\item exception: InvalidInputError -- thrown if the input is not a list of
\hyperref[expense_module]{Expense} objects
\end{itemize}
...
\begin{itemize}
  ...
  \href{https://github.com/PlutosCapstone/Plutos/blob/main/docs/SRS/SRS.pdf}{\textit{CATEGORIZATION\_ACCURACY}}\%.
\end{itemize}
...
\noindent calculate\_budget(history: List[Transaction], income: List[Income], budget: List[Budget]):
\begin{itemize}
\item precondition: The input is a list of transactions, a list of income, and a list of budgets.
\item transition: N/A
...
\item exception: 
    ...
    \end{itemize}
\item postcondition: The user is logged in and the session is active.
\end{itemize}

\noindent \textbf{createUserWithEmailAndPassword}(email: string, password: string):
\begin{itemize}
\item precondition: The user does not exist.
...
\item exception: 
    ...
    \end{itemize}
\item postcondition: The user now exists in the database.
\end{itemize}
