...
\begin{enumerate}
	...
	\begin{itemize}
		\item \textbf{GitHub Actions}: This tool can be set up to run workflows that execute tests, linters, and builds automatically on every pull request or commit, ensuring that code quality is maintained consistently (i.e. Husky). It can also be used to run daily sanity checks to ensure all features are working seamlessly after updates from the previous day.
		\item \textbf{Jenkins}: Jenkins is an open-source automation server that facilitates continuous integration and continuous deployment processes. It can be configured to automatically run tests, linting, and builds every time code is pushed to the repository, ensuring that code quality is maintained consistently. It can also be used to run daily sanity checks to ensure all features are working seamlessly after updates from the previous day.
	\end{itemize}
	\newpage
...
\begin{enumerate}
	...
	\noindent \begin{tabular}{|>{\raggedright\arraybackslash}p{3cm}|p{2cm}|>{\raggedright\arraybackslash}p{7cm}|}
		...
		\hline
		\textbf{Performance Testing} & 3 days & Conduct load and stress tests to evaluate system performance under expected load + high load \\
		\hline
...
\textbf{Control:} Automatic/Dynamic
...
understand core application features within \textit{MAX\_ACCOUNT\_ONBOARD\_TIME}
